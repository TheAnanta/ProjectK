<template>
    <div :style="{width: '100vw', height: isScrolled ? `${350 - Math.min(96, scrollY) / 2}px` : '350px', position: 'relative', padding: '20px', overflow: 'hidden',}">
        ...
        <div  style="display: flex; flex-direction: column; justify-content: start; align-items: start; position: absolute; bottom: 16px; left: 16px; z-index: 2; width: 100%; padding-right: 30px; gap: 12px;">
            ...
            <div :style="{display: scrollY >= 100 ? 'none' : 'flex', justifyContent: 'start', alignItems: 'start', gap: '10px', transitionProperty: 'all', transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)', transitionDuration: '150ms'}">
            ...
            <div :style="{width: '100%', display: scrollY >= 100 ? 'none' : 'flex', justifyContent: 'space-between', alignItems: 'center', transitionProperty: 'all', transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)', transitionDuration: '150ms'}">
                <p style="color: var(--primary-txt-color); font-size: 12px;">Lorem ipsum dolor sit amet, consectetur
                    adipiscing
                    elit, sed do eiusmod tempor</p>
                <v-btn prepend-icon="mdi-inbox-arrow-up-outline" rounded="xl" variant="outlined" style="border-color: var(--primary-border-color); color: var(--quadnary-color); font-size: 12px;">
                    View Analytics
                </v-btn>
            </div>
        </div>
